...
function updateScatterAngle() {
    ...
    ball2.Lab.spriteInstance.y = ball2.Lab.initr.y - Math.sin(initAngle)*(ball1.radius + ball2.radius)*0.125;
    ball2.CoM.spriteInstance.y = ball2.CoM.initr.y - Math.sin(initAngle)*(ball1.radius + ball2.radius)*0.125;
...
function preload() {
    ...
    if (window.devicePixelRatio >= 1.5 && canvasWidth<1500) {
        ...
        game.load.image('cofmPNG', 'assets/cofm@2x.png');
        game.load.image('p1m1m2', 'assets/p1m1m2latex@4x.png');
        game.load.image('p1', 'assets/p1latex@4x.png');
        game.load.image('pstar', 'assets/pstarlatex@4x.png');
        game.load.image('qstar', 'assets/qstarlatex@4x.png');
        game.load.image('q1', 'assets/q1latex@4x.png');
        game.load.image('q2', 'assets/q2latex@4x.png');
        game.load.image('u1', 'assets/u1latex@4x.png');
        game.load.image('v1', 'assets/v1latex@4x.png');
        game.load.image('v2', 'assets/v2latex@4x.png');
    } else {
        game.load.image('cofmPNG', 'assets/cofm.png');
    ...
    if(canvasWidth>=1500){
        textScaleDown = 2*window.devicePixelRatio;
    }else{
...
function create() {
    ...
    let style = {
        ...
        wordWrap: false,
        align: "left",
        backgroundColor: "rgba(235,238,238,0.7)"
    };
    ...
    vectorDiagramText = game.add.text(16, ( canvasHeight * 2 / 3 + 10 ), "Vector Diagram (scaled)", style);
...
function update() {
    ...
    if (isRunning){
        ...
        if( ((ball1.Lab.spriteInstance.visible == false)||(ball2.Lab.spriteInstance.visible == false)) && (ball1.CoM.spriteInstance.visible == false) && (ball2.CoM.spriteInstance.visible == false) ){
            $("#runButton").click();
            $("#runButton").click();
        }
